...
func (cs *CoinSet) TotalValue() (value dashutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue dashutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue dashutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount dashutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue dashutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue dashutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue dashutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue dashutil.Amount, coins []Coin) (Coins, error) {
...
// dashutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() dashutil.Amount {
	return dashutil.Amount(c.txOut().Value)
}
